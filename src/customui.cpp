...
namespace CustomUI {
    bool TextObject::create() {
        ...
        getLogger()->log_debug("TextObject::create: Creating gameObject: %s (object number: %i)", name.data(), counter);
        gameObj = New(GetClassFromName("UnityEngine", "GameObject"), createcsstr(name));

        // gameObj.SetActive(false);
        getLogger()->log_debug("TextObject::create: Setting gameObject.active to false");
        RET_0_UNLESS(RunMethod(gameObj, "SetActive", false));

        // gameObj.AddComponent<TextMeshProUGUI>();
        getLogger()->log_debug("TextObject::create: Getting type of TMPro.TextMeshProUGUI");
        auto *type_tmpugui = GetSystemType("TMPro", "TextMeshProUGUI");

        getLogger()->log_debug("TextObject::create: Adding component TMPro.TextMeshProUGUI");
        ...
        getLogger()->log_debug("TextObject::create: Getting type of TMPro.TMP_FontAsset");
        auto *type_fontasset = GetSystemType("TMPro", "TMP_FontAsset");
        getLogger()->log_debug("TextObject::create: Gotten the type!");
    ...
    void RawImageObject::monitorProgress(RawImageObject* obj) {
        getLogger()->log_info("monitorProgress start");
            ...
            if (auto progOpt = il2cpp_utils::RunMethod<float>(obj->WWW, "GetDownloadProgress")) {
                ...
                if (prog != prevProg) {
                    ...
                    if (cap != prevCap || prog < prevProg) {
                        getLogger()->log_info("prog is %i / %i", dataLen, cap);
                    }
                    getLogger()->log_info("wrAsyncOp progress: %f (raw = %p)", prog, *reinterpret_cast<void**>(&prog));
                    prevProg = prog;
        ...
        ss << std::endl;
        getLogger()->log_info("real progs (out of %i): %s", finalLen, ss.str().c_str());

        getLogger()->log_info("webRequest isDone.");
        auto* str = RET_V_UNLESS(RunMethod<Il2CppString*>(obj->WWW, "get_error"));
        if (str) {
            getLogger()->log_error("webRequest error: %s", to_utf8(csstrtostr(str)).c_str());
        } else {
            getLogger()->log_debug("webRequest had no error.");
        }
    }
    ...
    bool RawImageObject::textureWebRequestComplete(RawImageObject* obj, Il2CppObject* asyncOp) {
        ...
        getLogger()->log_debug("Entering textureWebRequestComplete!");
        ...
        if (!obj->parentTransform) {
            getLogger()->log_error("RawImageObject::textureWebRequestComplete: obj->parentTransform is null! Fix it!");
        }
        RET_0_UNLESS(RunMethodUnsafe(rawImageRectTransform, "SetParent", obj->parentTransform, false));

        // textMesh.rectTransform.anchorMin = anchorMin
        getLogger()->log_debug("RawImageObject::textureWebRequestComplete: Setting anchorMin");
        RET_0_UNLESS(RunMethod(rawImageRectTransform, "set_anchorMin", obj->anchorMin));

        // textMesh.rectTransform.anchorMax = anchorMax
        getLogger()->log_debug("RawImageObject::textureWebRequestComplete: Setting anchorMax");
        RET_0_UNLESS(RunMethod(rawImageRectTransform, "set_anchorMax", obj->anchorMax));

        // textMesh.rectTransform.sizeDelta = sizeDelta
        getLogger()->log_debug("RawImageObject::textureWebRequestComplete: Setting sizeDelta");
        RET_0_UNLESS(RunMethod(rawImageRectTransform, "set_sizeDelta", obj->sizeDelta));

        // textMesh.rectTransform.anchoredPosition = anchoredPosition
        getLogger()->log_debug("RawImageObject::textureWebRequestComplete: Setting anchoredPosition");
        ...
        getLogger()->log_debug("Callback success!");
